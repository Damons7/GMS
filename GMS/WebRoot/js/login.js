...
$(function() {
	...
	$("#username").blur(function() {
				...
				if (flag) {$("#reg_sp1").text(message).css('color', '#0000ff');
					// 判断是否可以正常注册
					//json数据：key:value,$("#username").val():用户名输入框的内容
					var json = {
						"username" : $("#username").val()
					};				
					$.get("http://localhost:8080/big_job/RegisterAjaxServlet",json, function(data, suss) {
								if (data == "true") {
									a="0";
								} else {
									$("#reg_sp1").text("用户名不存在").css('color','#f53808');a="1";
								}
							});
				} else {
...
$("#checkCode").blur(function() {
	var che = $("#checkCode").val();
	var message = "";
	var flag = false;
	if(che.length<0||che.length===0)
	{
	f = "6";// 修改全局变量
	message = "验证码为空！";
	}
